...
.. note::
  ...
  not include any of the page templates listed below (``registration/*.html``). It is
  left to programmers using ``django-password-policies-iplweb`` to create those
  templates for projects as needed.

  The three templates used to generate the password reset email
  (``password_reset_email.html``, ``password_reset_email.txt`` and
  ``registration/password_reset_subject.txt``) are the exception: unlike the page
  templates, a missing email template cannot be worked around by
  :mod:`django.contrib.admin` (which happens to ship usable, generic fallbacks for
  the other ``registration/*.html`` page templates), so this package ships minimal,
  generic defaults for them. A project can still override any of them by placing its
  own file at the same path earlier in ``TEMPLATES[0]['DIRS']``, or by passing
  ``email_template_name``/``email_html_template_name`` to
  :view:`PasswordResetFormView`.
...
* ``password_policies/password_reset_email.html``
    Used in the form :form:`PasswordResetForm` to send the password reset email.
    Defaults to a minimal, generic template shipped by this package (see the note
    above) — override by setting ``email_html_template_name`` on
    :view:`PasswordResetFormView` or by shadowing this path in your own
    ``TEMPLATES[0]['DIRS']``.
    ...
    + ``token``
        The token generated by the ``token_generator`` passed to the form, to use
        in a one-time secret URL.
    + ``uid``
...
* ``password_policies/password_reset_email.txt``
    Used in the form :form:`PasswordResetForm` to send the password reset email.
    Defaults to a minimal, generic template shipped by this package (see the note
    above) — override by setting ``email_template_name`` on
    :view:`PasswordResetFormView` or by shadowing this path in your own
    ``TEMPLATES[0]['DIRS']``.
...
* ``registration/password_reset_subject.txt``
    Used in the form :form:`PasswordResetForm` to send the password reset email.
    Not shipped by this package: :mod:`django.contrib.auth` already ships a working,
    generic default at this exact path, used automatically as long as
    ``django.contrib.auth`` is installed (it is, in virtually every Django project).
    Override by setting ``subject_template_name`` on :view:`PasswordResetFormView`
    or by shadowing ``registration/password_reset_subject.txt`` in your own
    ``TEMPLATES[0]['DIRS']``.
...
.. note::
  The default ``password_policies/password_reset_email.html`` and
  ``password_policies/password_reset_email.txt`` templates shipped by this package
  can be found under ``password_policies/templates/password_policies/`` and used as
  a starting point for a project's own, branded versions.
